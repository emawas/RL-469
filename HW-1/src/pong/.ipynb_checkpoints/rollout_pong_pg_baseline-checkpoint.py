...
from src.pong.value_cnn import ValueCNN  # if you don't have this, you can reuse PolicyCNN structure
...
def main():
    ...
    p.add_argument("--results_dir", type=str, default="results/pong_pg_baseline_rollouts")
    args = p.parse_args()
    ...
    plt.hist(returns, bins=20, color="steelblue", edgecolor="black", alpha=0.7)
    plt.title(f"Pong (Baseline) — {args.episodes} Rollouts\nMean={returns.mean():.2f}, Std={returns.std():.2f}")
