...
def main():
    ...
    ap.add_argument("--solve_score", type=float, default=500.0, help="MA100 score at which eps reaches eps_end")
    ap.add_argument("--eps_power", type=float, default=1.0, help="Shape factor for performance-based eps decay")
    ...
    for ep in range(1, args.episodes + 1):
        ...
        # --- Hybrid epsilon schedule (after episode) ---

        # MA100 for performance-based decay
        ma100 = moving_average(all_returns, 100)
        curr_ma = ma100[-1] if len(ma100) else 0.0
        
        # Monotonic best MA100 to smooth noise
        best_ma100 = max(best_ma100, curr_ma)
        
        # Progress toward solve target
        progress = min(best_ma100 / args.solve_score, 1.0)
        
        # Step-based decay (time-driven)
        eps_step = args.eps_end + (args.eps_start - args.eps_end) * max(0.0, 1 - global_step / args.eps_decay_steps)
        
        # Performance-based decay (learning-driven)
        eps_perf = args.eps_end + (args.eps_start - args.eps_end) * ((1.0 - progress) ** args.eps_power)
        
        # Keep the MOST exploratory epsilon
        eps = max(eps_perf, eps_step, args.eps_end)
        
        # Optional warm-up: force exploration until buffer has enough diversity
        if len(replay) < max(args.batch_size, 1000):
            eps = args.eps_start
        # per-episode:
